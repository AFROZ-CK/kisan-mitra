...
if (cropUploadForm) {
  cropUploadForm.addEventListener("submit", function(e) {
    e.preventDefault();
    diagnosisResult.style.display = "block";
    diagnosisResult.innerHTML = `
      <h3>Diagnosis Result</h3>
      <p><b>Status:</b> Leaf Blight (Detected)</p>
      <p><b>Treatment Recommendation:</b> Use a recommend fungicide, remove affected leaves, and monitor regularly.</p>
      <p><b>Suggested Fertilizer:</b> Urea 46%, Potash</p>
    `;
  });
}
...
if (fertilizerList) {
  // Dummy data
  const fertilizers = [
    {name: "Urea (46% N)", price: "₹350/bag", avail: "In Stock"},
    {name: "DAP (18-46-0)", price: "₹500/bag", avail: "In Stock"},
    {name: "Potash (MOP)", price: "₹420/bag", avail: "Limited"},
  ];
  fertilizerList.innerHTML = `
    <table>
      <tr><th>Name</th><th>Price</th><th>Stock</th><th>Buy</th></tr>
      ${fertilizers.map(f =>
        `<tr>
          <td>${f.name}</td>
          <td>${f.price}</td>
    ...
    <p style="font-style:italic; margin-top:8px;">* Demo only. Buying disabled.</p>
  `;
}

const sellCropForm = document.getElementById('sellCropForm');
const saleListings = document.getElementById('saleListings');
let cropsForSale = [];
if (sellCropForm) {
  sellCropForm.addEventListener("submit", function(e) {
    e.preventDefault();
    ...
    cropsForSale.push({name, qty, price});
    updateListings();
    sellCropForm.reset();
  });
}
function updateListings() {
  if (saleListings) {
    saleListings.innerHTML = `
      <h3>Your Crop Listings</h3>
      <ul>
        ${cropsForSale.map(c => `<li><b>${c.name}</b>: ${c.qty} kg @ ₹${c.price}/kg [Posted]</li>`).join("")}
      </ul>
    `;
  }
}
...
if (chatForm) {
  chatForm.addEventListener("submit", function(e) {
    ...
    setTimeout(() => {
      ...
      appendMsg('Kisan Assistant', 'For fertilizer selection, choose based on your soil type, crop, and current weather. Would you like a fertilizer recommendation?');
    }, 900);
    chatInput.value = '';
  });
}
function appendMsg(sender, msg) {
  if (chatWindow) {
    chatWindow.innerHTML += `<div><b>${sender}:</b> ${msg}</div>`;
